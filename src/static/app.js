document.addEventListener("DOMContentLoaded", () => {
      ...
      Object.entries(activities).forEach(([name, details]) => {
        ...
        if (details.participants && details.participants.length > 0) {
          ...
          details.participants.forEach((p) => {
            // include a delete icon with data attributes for activity and email
            participantsMarkup += `<li>${p} <span class="delete-icon" data-activity="${name}" data-email="${p}">🗑️</span></li>`;
          });
          participantsMarkup += "</ul>";
        } else {
          participantsMarkup += '<p class="no-participants">No one signed up yet.</p>';
        }
  ...
  // initialize event delegation for delete icons
